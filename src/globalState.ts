...
class GlobalState {
    ...
    public async setSolutionDetails(
        solutions: Record<string, CodeforcesSolution[]>,
    ) {
    ...
    public getProblemHtml(id: string): string | null {
        const cache = this.get(CODEFORCES_PROBLEMS_CACHE_KEY) as Record<
            string,
            string
        >;
        if (!cache || !cache[id]) {
    ...
    public async setProblemHtml(id: string, html: string) {
        let cache = this.get(CODEFORCES_PROBLEMS_CACHE_KEY) as Record<
            string,
            string
        >;
        if (!cache) {
