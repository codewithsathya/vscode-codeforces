...
async function getHandleSolutions(username: string) {
    try {
        ...
        const { data: handleDetails }: any = await axios.get(`https://codeforces.com/api/user.status?handle=${username}`);
        for(const submission of handleDetails.result) {
            ...
            if(!result[problemId] || (result[problemId] && result[problemId].timeSubmitted < timeSubmitted)) {
                result[problemId] = {
                    username,
                    contestId,
                    index,
...
export async function saveSolutionDetails() {
    ...
    try {
        for(const handle of handles) {
            ...
            codeforcesChannel.appendLine(`Collected solution details of ${handle}`);
            await sleep(1000);
        }
