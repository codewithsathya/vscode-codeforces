...
export class CodeforcesProblemParser {
    ...
    private parseMainProblem(html: string, url: string, task: TaskBuilder): void {
        try {
            ...
            const titleElement = elem.querySelector('.problem-statement > .header > .title');
            ...
            const memoryLimitStr = memoryLimitNode?.textContent?.split(' ')[0] || '0';
            ...
            codeforcesChannel.appendLine(`Error occurred at parsing problem: ${error}`);
        }
    }
    ...
    private parseAcmSguRuProblemInsideTable(html: string, task: TaskBuilder): void {
        ...
        const timeLimitMatch = /time limit per test: ([0-9.]+)\s+sec/.exec(html);
        if (timeLimitMatch) {
            task.setTimeLimit(parseFloat(timeLimitMatch[1]) * 1000);
        }
        const memoryLimitMatch = /memory\s+limit per test:\s+(\d+)\s+KB/.exec(html);
        if (memoryLimitMatch) {
        ...
        for (let i = 0; i < blocks.length - 1; i += 2) {
            task.addTest(blocks[i].textContent as string, blocks[i + 1].textContent as string);
        }
    }
    ...
    private parseAcmSguRuProblemNotInsideTable(html: string, task: TaskBuilder): void {
        ...
        const timeLimitMatch = /Time\s+limit per test: ([0-9.]+)\s+sec/i.exec(html);
        if (timeLimitMatch) {
            task.setTimeLimit(parseFloat(timeLimitMatch[1]) * 1000);
        }
        const memoryLimitMatch = /Memory\s+limit(?: per test)*: (\d+)\s+(?:kilobytes|KB)/i.exec(html);
        if (memoryLimitMatch) {
            task.setMemoryLimit(parseInt(memoryLimitMatch[1], 10) / 1000);
        }
        ...
        elem.querySelectorAll('table').forEach((table: Element) => {
            ...
            if (blocks.length === 4) {
                task.addTest(blocks[2].textContent as string, blocks[3].textContent as string);
            }
        });
    }
    ...
    private getLastTextNode(elem: Document, selector: string): ChildNode {
        ...
        const textNodes = [...selectedNode.childNodes].filter(node => node.nodeType === 3);
